...
import {
  ...
  AlertTriangle, CheckCircle2, Users, DollarSign, Wrench, HelpCircle,
...
export default function SummaryPage() {
  ...
  // active date — defaults to today, can be overridden via ?date=YYYY-MM-DD
  ...
  const handleRegenerate = async () => {
    ...
    setRegen(true);
    try {
      const data = await generateTodaySummary();
      setSummary(data);
    } catch {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({ title: 'OpsFly Daily Summary', text });
    } else {
    ...
    <>
      ...
      <main className="page" style={{ paddingTop: 16 }}>
        ...
        <div style={{
          ...
          <button onClick={() => changeDate(-1)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex' }}>
            <ChevronLeft size={20} />
          </button>
          ...
          <button
            ...
            disabled={isFuture(new Date(activeDate.getTime() + 86400000))}
            style={{ background: 'none', border: 'none', color: isFuture(new Date(activeDate.getTime() + 86400000)) ? 'var(--border)' : 'var(--text-muted)', cursor: isFuture(new Date(activeDate.getTime() + 86400000)) ? 'not-allowed' : 'pointer', display: 'flex' }}
          >
            <ChevronRight size={20} />
          </button>
        </div>

        {/* ── Action row (Regenerate + Share) ── */}
        ...
        {!loading && !error && summary && summary.totalIssues === 0 && (
          <div style={{ textAlign: 'center', padding: '40px 24px', background: 'var(--bg-card)', borderRadius: 20, border: '1px solid var(--border)' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>No notes were recorded on this day.</p>
          </div>
        )}
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{
                ...
                {summary.totalTasks === 0 ? 'None' : summary.completedTasks === summary.totalTasks ? 'All Done!' : `${Math.round((summary.completedTasks / summary.totalTasks) * 100)}%`}
              </div>
            </div>
            ...
            <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textAlign: 'center' }}>
              Generated {summary.generatedAt ? new Date(summary.generatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
            </p>
          </>
        )}
      </main>
