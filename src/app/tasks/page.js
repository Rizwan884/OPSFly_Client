...
import { getTasks, createTask, completeTask, reopenTask, deleteTask, invalidateTasksCache } from '@/src/services/api';
...
// ── TaskCard ──────────────────────────────────────────────────────────────────
function TaskCard({ task, onComplete, onReopen, onDelete }) {
    ...
    <div style={{
      ...
      >
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <label style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block', marginBottom: 8 }}>
              Task Title
            </label>
            <input
              autoFocus
          ...
          <button
            ...
            className="confirm-btn" style={{ marginTop: 8 }}
          >
...
export default function TasksPage() {
  ...
  const fetchTasks = useCallback(async (force = false) => {
    try {
      const data = await getTasks(force);
      setTasks(data);
    } catch (err) {
  ...
  const handleReopen = async (id) => {
    setTasks(prev => prev.map(t => t._id === id ? { ...t, status: 'open', completedAt: null } : t));
    await reopenTask(id);
    invalidateTasksCache();
  };
  ...
  const handleAddTask = async (data) => {
    ...
    setTasks(prev => {
      ...
      updated.sort((a, b) => priorityOrder(a.priority) - priorityOrder(b.priority) || new Date(b.createdAt) - new Date(a.createdAt));
      return updated;
    });
  };
    ...
    <>
        ...
        <div style={{
          ...
          {[
            ...
            <button
              ...
              onClick={() => setTab(key)}
              style={{
                flex: 1, padding: '10px 0', borderRadius: 9, fontWeight: 700,
            ...
            {openTasks.length === 0 ? (
              <div style={{
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>
                  No open tasks. Record a note to auto-generate tasks.
                </p>
              </div>
            ) : (
                ...
                return (
                  <div key={priority}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {group.map(task => (
                        <TaskCard key={task._id} task={task} onComplete={handleComplete} onReopen={handleReopen} onDelete={handleDelete} />
                      ))}
                    </div>
                  </div>
                );
      ...
      <button
        ...
        style={{
          ...
          boxShadow: '0 8px 24px rgba(29,123,255,0.4)',
      ...
      {/* ── Menu item styles (inline in JS since no CSS modules) ── */}
      <style>{`
        .menu-item {
